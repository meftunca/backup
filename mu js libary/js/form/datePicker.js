var datepicker = (function () {
    ...
    function input_val(params) {
        return (params.value = day + "-" + month+1 + "-" + year);
    }
    ...
    Array.prototype.forEach.call(datepicker, function (el) {
        ...
        Array.prototype.forEach.call(menu, function (summary) {
            ...
            let classL = el.classList;
             document.addEventListener("click", function (event) {
                var isClickInside = that.contains(event.target) ||el.contains(event.target) || summary.contains(event.target);
                if (!isClickInside) {
